...
export default ({session}) => {
    ...
    const { data, isError, isLoading } = useSWR(`/reports/get`, fetcher)
    if (isLoading) return;
    dispatch(isFileActions.set(false))
    const title = "إضافة تقرير | التقارير";
    return (
        <>
            <Head>
            ...
            <section className={Styles.index}>
                <HeaderPages title={'إضافة تقرير'} routeBack={'/reports'} />
                <div>
                    {
                        session.user.role === 'admin' && <IsFile />
                    }
                    <FormUser session={session} clubs={data.clubs} isAddReports={true} />
                </div>
            </section>
        </>
    )
