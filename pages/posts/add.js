...
export default ({session}) => {
    ...
    const { data, isError, isLoading } = useSWR(`/posts/get`, fetcher)
    if (isLoading) return;
    dispatch(isFileActions.set(false))
    const title = "إضافة منشور | المنشورات";
    return (
        <>
            <Head>
            ...
            <section className={Styles.index}>
                <HeaderPages title={'إضافة منشور'} routeBack={'/posts'} />
                <div>
                    {
                        session.user.role === 'admin' && <IsFile />
                    }
                    <FormUser session={session} clubs={data.clubs} isAddPosts={true} />
                </div>
            </section>
        </>
    )
