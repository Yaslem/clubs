...
export default () => {
    const title = "قائمة الجوائز | الأدوات";
    const router = useRouter()
    const { data, isError, isLoading } = useSWR(`/tools/awards?page=${router.query.page || 1}`, fetcher)
    return (
        <>
            <Head>
            ...
            <section className={Styles.index}>
                <HeaderPages title={'قائمة الجزائز'} route={'/add'} add={'إضافة جائزة'} routeBack={'/tools'} />
                {
                    isLoading
                        ? <Loading />
