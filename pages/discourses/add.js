...
export default ({session}) => {
    ...
    const { data, isError, isLoading } = useSWR(`/discourses/get`, fetcher)
    if (isLoading) return;
    dispatch(isFileActions.set(false))
    const title = "رفع خطاب | الخطابات";
    return (
        <>
            <Head>
            ...
            <section className={Styles.index}>
                <HeaderPages title={'رفع خطاب'} routeBack={'/discourses'} />
                <div>
                    {
                        session.user.role === 'admin' && <IsFile />
                    }
                    <FormUser session={session} clubs={data.clubs} isAddDiscourses={true} />
                </div>
            </section>
        </>
    )
